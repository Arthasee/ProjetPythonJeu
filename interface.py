...
class Interface:
    def __init__(self, game):
        ...
        self.ath_background = pygame.image.load("hud.png")  # Charge l'image de fond pour l'ATH
    ...
    def draw_ath(self):
        ...
        font_pokemon = pygame.font.Font(font_path, 22)
        ...
        for player_pokemon in self.game.player_units:
            ...
            text_surface = font_pokemon.render(health_text, True, BLACK)  # Couleur noir pour les joueurs
        ...
        for enemy_pokemon in self.game.enemy_units:
            ...
            text_surface = font_pokemon.render(health_text, True, WHITE)   
            text_rect = text_surface.get_rect(center=(center_x, y_offset))  # Centrer le texte
            self.game.screen.blit(text_surface, text_rect)
            y_offset += 20
        ...
        if self.game.player_action_points > 0:
            action_points_text = font_medium.render(f"Il te reste {self.game.player_action_points} points d'action !", True, BLACK)
        else:
    ...
    def get_health_bar_color(self, current_hp, max_hp):
        ...
        if ratio > 0.7:
            return GREEN  # Vert pour une bonne santé
        elif ratio > 0.3:
            return ORANGE # Jaune pour une santé moyenne
        else:
            return RED  # Rouge pour une faible santé
